#!/usr/bin/env python3
"""
grade_results.py — World Cup signal grading.

Fetches completed game scores from The Odds API and grades any open
soccer_signals rows in the DB.
...
from .fetch_signals import fetch_wc_scores, SPORT
...
def run(days_back: int = 3) -> None:
    print("=" * 55)
    print("  ACE — World Cup Grade Results")
    ...
    try:
        score_games = fetch_wc_scores(days_back)
    ...
    # Deduplicate by game_id — grade each game only once
    ...
    for sig in open_sigs:
        ...
        # Fallback: team name match when ID doesn't match
        if game is None:
            for g in completed:
        ...
        for r in results:
            ...
            print(
                ...
                f"{market_str}  edge={r['edge_pp']*100:.1f}pp"
            )
