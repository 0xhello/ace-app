    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0b0a]">
      <div className="max-w-[1200px] mx-auto px-6 py-7 space-y-5">
        ...
        <div className="ace-panel p-5">
          ...
          <div className="flex gap-0.5 mb-5 border-b border-[#181c18]">
            {(["all","bets","conf","source"] as const).map(t => (
              <button key={t} onClick={() => setPerfTab(t)}
                ...
                {t === "all" ? "All picks" : t === "bets" ? "Bets only" : t === "conf" ? "By confidence" : "Pinnacle vs fallback"}
              </button>
            ))}
          </div>
          ...
          {perfTab === "conf" && m && (
            <div>
              <p className="text-[10px] text-[#4a524a] mb-5">Win rate by model confidence — graded picks only</p>
              {m.buckets.map(b => (
          ...
          {perfTab === "source" && m && (
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { label: "Fallback",        sub: "confidence threshold only", wins: m.fallbackWins, graded: m.fallbackGraded, wr: m.fallbackWinRate },
