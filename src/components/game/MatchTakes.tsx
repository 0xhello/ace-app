...
const TIER_STYLE: Record<TakeTier, { chip: string; dot: string; label: string }> = {
  ...
  Pass:   { chip: "bg-[#1c1614] text-[#c98b7a] ring-[#3a2620]", dot: "bg-[#a86a55]", label: "Pass" },
};
const TIER_ORDER: Record<TakeTier, number> = { Strong: 0, Lean: 1, Slight: 2, Pass: 3 };
...
export default function MatchTakes({ takes }: { takes: GameTakes | null }) {
  ...
  const isAgent = takes?.source === "agent";
  const plays = list.filter((t) => t.tier !== "Pass").length;
    ...
    <section className="mt-5 rounded-2xl border border-[#2c4a2f] bg-gradient-to-b from-[#0f1a10] to-[#0c0e0c] p-5">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          {isAgent && (
            <span className="rounded-md bg-[#16331f] ring-1 ring-[#215a2e] px-1.5 py-0.5 text-[8.5px] font-bold uppercase tracking-[0.12em] text-[#5fe39a]">Analyst</span>
          )}
        </div>
        {list.length > 0 && (
          <span className="text-[10px] font-mono text-[#7a8278]">{plays} play{plays === 1 ? "" : "s"}{list.length > plays ? ` · ${list.length - plays} pass` : ""}</span>
        )}
      </div>

      {takes?.summary && (
        <p className="mb-3 text-[13px] text-[#d7dad2] leading-snug border-l-2 border-[#3ee68a]/40 pl-3">{takes.summary}</p>
      )}
      ...
      {list.length > 0 ? (
        <>
                ...
                <div key={i} className="flex items-start gap-3 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-baseline gap-2 flex-wrap">
                      ...
                      <span className={`text-[14px] font-semibold ${passed ? "text-[#9ca39a]" : "text-white"}`}>{t.selection}</span>
                      {t.model_pct != null && (
                        <span className="text-[11px] font-mono text-[#7f8a78]">{t.model_pct}%</span>
                      )}
                      {t.odds && (
                        <span className="text-[10.5px] font-mono text-[#5fe39a] bg-[#0e1a10] ring-1 ring-[#1d3320] rounded px-1.5 py-0.5">{t.odds}</span>
                      )}
                    </div>
                    {t.reasons?.[0] && (
          ...
          <p className="mt-3 pt-3 border-t border-[#16331f]/60 text-[10px] text-[#5f655c] leading-relaxed">
            {isAgent
              ? "ACE Analyst weighs the match model, form, injuries, corners and player data, then makes the call — graded against results, not a guarantee. Prices shown are read at publish; shop your own book."
